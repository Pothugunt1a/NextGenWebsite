...
>(({ className, children, image, imageAlt = "Dropdown image", sideOffset = 8, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        "z-50 min-w-[400px] max-w-[600px] overflow-hidden rounded-lg bg-gray-900/95 backdrop-blur-sm shadow-xl border border-gray-700 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
        className
      )}
      {...props}
    >
      <div className="flex min-h-[200px]">
        ...
        <div className="flex-1 border-l-4 border-primary pl-6 pr-4 py-6 text-white">
          {children}
        </div>
        {/* Right side image */}
        {image && (
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    ref={ref}
    className={cn("px-0 py-1.5 text-sm font-semibold text-white", className)}
    {...props}
  />
))
